...
This library is using [Ktor](https://github.com/ktorio/ktor) server with [Netty](https://github.com/netty/netty) in Android. `react-native-echo` is currently doing good with the Ktor + Netty server library with really minimal error, and the `multipart/form-data` request handling with cached files performs still good, but the performance in a Android device is becoming a question even from myself, is this good or not.
...
| POST /api/formdata      | I sent one entry with string value, and two entry files. The server (RN side) is doing nothing about the files. | 783            | 11.55           | 681                     | 82                     | 7441                   | 1343        | 1606       | 3266        | 0                |
...
I also plan to use [uWebSockets](https://github.com/uNetworking/uWebSockets) server in Android, and probably with [NitroModules](https://github.com/mrousavy/nitro), if they can perform better than the current implementation with Ktor+Netty server with React Native Turbo Module.
